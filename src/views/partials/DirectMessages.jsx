    ...
    <div className={styles.messagesCont}>
      ...
      <ul className={styles.directMessageUl}>
        {directMessages.map((msg, indx) => {
          return (
            <li key={indx} className={styles.msgLi}>
              <div className={styles.msgProfileCont}>
                <img
                  onClick={(e) => handleClickOnProfileImg(e, msg.author.id)}
                  className={styles.msgProfilePicture}
                  src={`${import.meta.env.VITE_FETCH_BASE_URL}/${
                    msg.author.profileImgFilePath
                  }`}
                  alt={`${msg.author.profileName}'s Profile Picture`}
                  width={"25px"}
                  height={"25px"}
                />
                <p className={styles.profileName}>
                  {he.decode(msg.author.profileName)}
                </p>
                <p>
                  {formatDistance(msg.createdAt, new Date(), {
                    addSuffix: true,
                  })}
                </p>
              </div>
              <p className={styles.chatMsgContent}>
                {he.decode(msg.messageContent)}
              </p>
              {!msg.attatchedImagePath ? null : <MessageImage msg={msg} />}
            </li>
          );
        })}
        <li ref={endOfMsg}></li>
